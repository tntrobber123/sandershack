	...
	def on_render(self, console: tcod.Console) -> None:
		...
		if number_of_items_in_inventory > 0:
			for i, item in enumerate(self.engine.player.inventory.items):
				...
				print(quantity.Quantifiable.modify(entity.Item.quantifiable.quantifiable, 0))
				if entity.Item.quantifiable.quantifiable > 1:
					item_string = f"({item_key}) {item.name} * {entity.Item.quantifiable.quantifiable}"
...
class SelectIndexHandler(AskUserEventHandler):
	...
	def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
		...
		if key in MOVE_KEYS:
			...
			if event.mod & (tcod.event.KMOD_LSHIFT | tcod.event.KMOD_LSHIFT):
				modifier *= 50
